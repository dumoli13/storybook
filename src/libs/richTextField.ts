import {
  ...
  RICH_ELEMENT_TYPE,
  TEXT_TAG,
} from '../types/inputs/richTextField';
...
export function deserializeHTMLFromWord(html: string): any[] {
  ...
  const walkNode = (el: ChildNode): any => {
    ...
    if (new RegExp(/font-weight:\s*bold/i).exec(style))
      children = children.map((c) => ({ ...c, bold: true }));
    if (new RegExp(/font-style:\s*italic/i).exec(style))
      children = children.map((c) => ({ ...c, italic: true }));
    if (new RegExp(/text-decoration:\s*underline/i).exec(style))
      children = children.map((c) => ({ ...c, underline: true }));
    if (new RegExp(/text-decoration:\s*line-through/i).exec(style))
